/**
 * 🔥 SAAT BAŞI PREMIUM TELEGRAM SİNYALLERİ
 *
 * Bu servis her saat başı çalışır ve:
 * - Premium Signals API'yi çağırır
 * - En iyi 5 coini bulur
 * - Giriş/Çıkış/TP/SL bilgilerini hesaplar
 * - Telegram'a Türkçe bildirim gönderir
 *
 * PM2 ile çalıştırılır: pm2 start hourly-premium-signals.js --cron "0 * * * *"
 */
...
async function fetchPremiumSignals() {
  return new Promise((resolve, reject) => {
    const url = `${API_BASE_URL}/api/telegram/premium-signals`;
...
async function main() {
  ...
  console.log('\n╔════════════════════════════════════════════════════════╗');
  console.log('║  🔥 SAAT BAŞI PREMIUM SİNYALLER - BAŞLADI            ║');
  console.log('╚════════════════════════════════════════════════════════╝');
  console.log(`⏰ Başlangıç: ${new Date().toLocaleString('tr-TR')}\n`);

  try {
    // 1. Fetch signals from API
    console.log('📡 Premium Signals API\'ye bağlanılıyor...');
    const signalData = await fetchPremiumSignals();

    console.log(`✅ API yanıt verdi:`);
    console.log(`   - Taranan coin sayısı: ${signalData.totalAnalyzed}`);
    console.log(`   - Bulunan sinyal sayısı: ${signalData.totalFound}`);
    console.log(`   - Geçen süre: ${signalData.elapsedTimeMs}ms\n`);

    // 2. Get Telegram message
    const telegramMessage = signalData.telegramMessage;
    ...
    console.log('\n╔════════════════════════════════════════════════════════╗');
    console.log('║  ✅ SAAT BAŞI PREMIUM SİNYALLER - TAMAMLANDI         ║');
    console.log('╚════════════════════════════════════════════════════════╝');
    console.log(`📊 Özet:`);
    ...
    try {
      const errorMessage = `⚠️ *PREMIUM SİNYAL SİSTEMİ HATASI*\n\n` +
        `❌ Hata: ${error.message}\n` +
...
module.exports = { main, fetchPremiumSignals, sendTelegramMessage };
