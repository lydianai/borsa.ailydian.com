 ...
 * - Dynamic notifications (her bildirim farklı)
 * - Clear all button (Temizle)
 * - Compact size (Küçük)
 * - Sort by confidence % (En yüksek yüzde üstte)
 * - Auto-remove low confidence (Yüzde düşerse kaldır)
 */
...
const MIN_CONFIDENCE_THRESHOLD = 70; // Bu değerin altındakiler otomatik kaldırılır
...
export function NotificationPanel({ isOpen = false, onClose }: NotificationPanelProps = {}) {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      // Process AI signals FIRST (öncelik AI'da)
      if (aiSignals.success && aiSignals.data.signals) {
        aiSignals.data.signals
          ...
          .forEach((signal: any) => {
            ...
            // AL sinyalleri için özel mesaj
            const confidenceEmoji = signal.confidence >= 90 ? '🔥' : signal.confidence >= 80 ? '⚡' : '💎';

            newNotifications.push({
              id: `${signal.id}-${now}`,
              type: signal.type,
              symbol: signal.symbol,
              message: `${confidenceEmoji} ${signal.symbol}: Güçlü AL sinyali - %${signal.confidence} güven`,
      ...
      // SIRALA: En yüksek confidence üstte, KALICI
      newNotifications.sort((a, b) => {
        // Önce confidence'e göre sırala (en yüksek en üstte)
        if (b.confidence !== a.confidence) {
          return b.confidence - a.confidence;
        }
        // Confidence eşitse timestamp'e göre (en yeni en üstte)
        return b.timestamp.getTime() - a.timestamp.getTime();
      });

      // Sadece confidence threshold üzerinde olanları tut (otomatik temizleme)
      const filteredNotifications = newNotifications.filter(
        n => n.confidence >= MIN_CONFIDENCE_THRESHOLD
      );

      // Maksimum 15 bildirim göster
  ...
  useEffect(() => {
    fetchNotifications();
    const interval = setInterval(fetchNotifications, 5000); // Update every 5s (daha sık)
  ...
  // TEMİZLE butonu - tüm bildirimleri kaldır
  ...
  const handleEnablePush = async (event?: React.MouseEvent) => {
    ...
    try {
      if (pushSubscribed) {
        ...
        console.log('[NotificationPanel] ✅ Push notifications disabled');
      } else {
        // Subscribe
        await PushClient.subscribeToPush();
        setPushSubscribed(true);
        console.log('[NotificationPanel] ✅ Push notifications enabled');
      }
    } catch (error: any) {
      console.error('[NotificationPanel] ❌ Push toggle failed:', error);
      setPushError(error.message || 'Push notification işlemi başarısız');
    } finally {
  ...
  const handleTestPush = async (event?: React.MouseEvent) => {
    ...
    if (!pushSubscribed) {
      setPushError('Önce push bildirimlerini aktif etmelisiniz');
      return;
    }
    ...
    try {
      ...
      console.error('[NotificationPanel] ❌ Test notification failed:', error);
      setPushError(error.message || 'Test bildirimi gönderilemedi');
    } finally {
      setPushLoading(false);
    }
  };
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    ...
                    {/* Confidence Badge - Kalıcı gösterge */}
                    <span style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              >
                {pushLoading ? 'Yükleniyor...' : pushSubscribed ? 'AKTİF' : 'AKTİF ET'}
              </button>
            </div>
